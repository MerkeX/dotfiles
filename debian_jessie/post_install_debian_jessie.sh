...
sudo apt-get install linux-headers-$(uname -r) build-essential curl menulibre snap apt-transport-https execstack git debhelper dkms lib32gcc1 gdebi sudo software-properties-common libxss1 libappindicator1 libindicator7 qmlscene qt5-qmake qt5-default qtdeclarative5-dev -y &&
...
echo "deb http://nl.mxrepo.com/mx/repo/ mx15 main" | sudo tee /etc/apt/sources.list.d/mx15.list &&
...
sudo apt-get install build-essential libssl-dev cmake libgtkmm-3.0-dev gettext libarchive-dev -y  &&
...
sudo dpkg -i mailspring-1.8.0-amd64.deb &&
