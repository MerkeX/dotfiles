#!/bin/bash
echo ':: Executing postInstall script for "Zorin OS"...' && sleep 2
...
# LUTRIS
echo "deb http://download.opensuse.org/repositories/home:/strycore/Debian_12/ ./" | \
sudo tee /etc/apt/sources.list.d/lutris.list
wget -q https://download.opensuse.org/repositories/home:/strycore/Debian_12/Release.key -O- | \
sudo apt-key add -
...
sudo apt-get install brasero cmus dhcpcd5 dosfstools gedit gmtp openbox picom \
...
playonlinux polybar pulseaudio qbittorrent radeontop rclone signal-desktop \
spotify-client steam stellarium synaptic teams-for-linux telegram-desktop terminator \
